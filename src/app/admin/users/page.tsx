...
import { bookings, users as initialUsers } from "@/lib/mock-data";
...
export default function AdminUsersPage() {
  ...
  return (
    <AppShell
      ...
      title="Users"
      description="Search users, filter by role, review booking activity, and enable or disable access."
    >
          ...
          <div className="flex flex-wrap gap-2">
            {roleFilters.map((filter) => (
              <button
                key={filter}
                className={cn(
                  "min-h-10 rounded border px-3 text-sm font-bold",
                  role === filter ? "border-signal-success bg-signal-success text-ink-950" : "border-white/10 bg-ink-850 text-[#C9C9DA]"
                )}
      ...
      <section className="overflow-hidden rounded-lg border border-white/10 bg-ink-900">
        ...
        {filteredUsers.map((user) => (
          <article key={user.id} className="grid gap-3 border-b border-white/10 px-4 py-4 last:border-b-0 lg:grid-cols-[1.2fr_120px_1fr_120px_120px_140px] lg:items-center">
            ...
            <span className={cn("w-fit rounded border px-2 py-1 text-xs font-bold", user.isActive ? "border-signal-success/40 bg-signal-success/10 text-signal-success" : "border-signal-danger/40 bg-signal-danger/10 text-signal-danger")}>
              {user.isActive ? "Enabled" : "Disabled"}
            </span>
            <div className="flex gap-2">
              <button className="grid min-h-10 w-10 place-items-center rounded border border-white/10 bg-ink-850 hover:bg-white/5" onClick={() => setSelectedUser(user)} title="View booking history">
                <Eye size={17} aria-hidden="true" />
              </button>
              <button className="grid min-h-10 w-10 place-items-center rounded border border-white/10 bg-ink-850 hover:bg-white/5" onClick={() => toggleUser(user.id)} title={user.isActive ? "Disable user" : "Enable user"}>
