...
export default function AdminResourcesPage() {
  ...
  const filteredResources = useMemo(() => {
    ...
    return managedResources.filter((resource) => {
      ...
      const matchesActive =
        active === "all" || (active === "active" ? resource.isActive : !resource.isActive);
  ...
  function deactivateResource() {
    ...
    setManagedResources((current) =>
      current.map((resource) =>
        resource.id === confirmTarget.id ? { ...resource, isActive: false } : resource
      )
    );
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <label className="flex min-h-11 flex-1 items-center gap-2 rounded border border-white/10 bg-ink-850 px-3 text-sm text-[#A0A0B8]">
            ...
            <input
              ...
              placeholder="Search by resource, department, type, or location"
              value={search}
      ...
      {isFormOpen ? (
        <section className="mb-5 rounded-lg border border-white/10 bg-ink-900 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="grid gap-2 text-sm font-bold text-[#C9C9DA]">
              ...
              <select
                className="min-h-11 rounded border border-white/10 bg-ink-850 px-3 text-white outline-none"
                value={form.type}
                onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as ResourceType }))}
              >
                {typeFilters.filter((filter) => filter !== "all").map((filter) => (
            ...
            <label className="grid gap-2 text-sm font-bold text-[#C9C9DA]">
              ...
              <select
                ...
                value={form.status}
                onChange={(event) =>
                  setForm((current) => ({ ...current, status: event.target.value as Resource["status"] }))
                }
              >
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end">
            <button
              className="inline-flex min-h-10 items-center justify-center gap-2 rounded border border-white/10 bg-white/[0.03] px-4 text-sm font-bold hover:bg-white/10"
              onClick={() => setIsFormOpen(false)}
            >
            ...
            <button
              className="inline-flex min-h-10 items-center justify-center gap-2 rounded bg-signal-success px-4 text-sm font-bold text-ink-950 hover:bg-white"
        ...
        {filteredResources.map((resource) => (
          ...
          >
            <div>
              ...
              <p className="text-sm text-[#A0A0B8]">{resource.schedule}</p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {resource.isActive ? (
                <button
                  className="grid min-h-10 w-10 place-items-center rounded border border-signal-danger/40 bg-signal-danger/10 text-signal-danger hover:bg-signal-danger/20"
                  onClick={() => setConfirmTarget(resource)}
                  title={`Deactivate ${resource.name}`}
                >
                  <Power size={17} aria-hidden="true" />
                </button>
              ) : (
                <button
                  className="inline-flex min-h-10 items-center gap-2 rounded border border-signal-success/40 bg-signal-success/10 px-3 text-sm font-bold text-signal-success hover:bg-signal-success/20"
                  onClick={() => activateResource(resource.id)}
                >
                  <Power size={17} aria-hidden="true" />
                  Activate
                </button>
        ...
        <div className="fixed inset-0 z-30 grid place-items-center bg-ink-950/80 px-4 backdrop-blur-sm">
          <section className="w-full max-w-md rounded-lg border border-white/10 bg-ink-900 p-5 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-[#A0A0B8]">
              {confirmTarget.name} will be hidden from active scheduling while remaining visible in admin records.
            </p>
            <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
              <button
                className="inline-flex min-h-10 items-center justify-center rounded border border-white/10 bg-white/[0.03] px-4 text-sm font-bold hover:bg-white/10"
                onClick={() => setConfirmTarget(null)}
              >
                Cancel
              </button>
              <button
                className="inline-flex min-h-10 items-center justify-center rounded bg-signal-danger px-4 text-sm font-bold text-white hover:bg-signal-danger/80"
